...
def build_dataset_live(fields, documents,
    ...
    dataset = LiveTextDataset(fields, documents,
                              seq_length=seq_length,
                              seq_length_trunc=seq_length_trunc,
                              dynamic_dict=dynamic_dict,
                              use_filter_pred=use_filter_pred)
    return dataset
...
def _sents2feats(lines, max_len, side):
...
def _read_text_file(path, truncate, side):
    ...
    with codecs.open(path, "r", "utf-8") as corpus_file:
        for output in _sents2feats(corpus_file, truncate, side):
            yield output


def _make_example(path, truncate, side, read_from_file=True):
    """
    ...
    if read_from_file:
        ...
        documents = path
        examples = _sents2feats(documents, truncate, side)
...
class LiveTextDataset(TextDataset):
    def _process_corpus(self, fields, documents,
                        seq_length=0,
                        seq_length_trunc=0,
                        dynamic_dict=True, use_filter_pred=True):
        """
        ...
        Args:
            fields: a dictionary of Field objects. Keys are like 'src',
                    'tgt', 'src_map', and 'alignment'.
            documents: an iterable of string documents
            seq_length: maximum source sequence length.
            seq_length_trunc: truncated source sequence length.
            dynamic_dict: create dynamic dictionaries?
            use_filter_pred: use a custom filter predicate to filter examples?
        ...
        # Process the corpus into examples, and extract number of features,
        # if any. Note tgt_path might be None.
        examples, self.n_src_feats = \
            _make_example(documents, seq_length_trunc, "src",
        ...
        def filter_pred(example):
            return 0 < len(example.src) <= seq_length
